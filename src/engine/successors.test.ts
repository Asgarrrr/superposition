...
const moveTo = (out: [Input, GameState][], dir: Pos) =>
  out.find(
    ([inp]) =>
      inp.kind === "move" && inp.dir[0] === dir[0] && inp.dir[1] === dir[1],
  )?.[1];
...
describe("resolveMove — au plus une mécanique de déplacement", () => {
  it("deux mécaniques actives avec resolveMove → erreur explicite", () => {
    ...
    try {
      ...
      delete MECHANICS.fusion.hooks.resolveMove;
    }
  });
});
...
describe("applyInput", () => {
  ...
  it("rejoue exactement le successeur correspondant", () => {
    const next = applyInput(initialState(level), level, {
      kind: "move",
      dir: [1, 0],
    });
  ...
  it("renvoie null pour un input sans effet", () => {
    ...
    expect(
      applyInput(initialState(corner), corner, { kind: "move", dir: [-1, 0] }),
    ).toBeNull();
    expect(
      applyInput(initialState(corner), corner, { kind: "split", dir: [1, 0] }),
    ).toBeNull();
  });
});
