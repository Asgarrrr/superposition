...
import { hashState, initialState, isWin, merges } from "./state.ts";
...
describe("merges", () => {
  const withFusion = testLevel({
    size: 3,
    mods: ["fusion"],
    a: { start: [0, 0], goal: [2, 2], walls: [] },
    b: { start: [0, 1], goal: [2, 2], walls: [] },
  });
  const without = testLevel({ ...withFusion, mods: [] });

  it("l’équation se juge en coordonnées PLATEAU : a == b + off", () => {
    expect(merges(withFusion, [1, 2], [1, 1], [0, 1])).toBe(true);
    expect(merges(withFusion, [1, 1], [1, 1], [0, 1])).toBe(false);
  });

  it("exige une mécanique qui active la fusion", () => {
    expect(merges(without, [1, 1], [1, 1], [0, 0])).toBe(false);
  });
});
...
describe("hashState", () => {
  it("est injectif sur tous les états atteignables", () => {
    ...
    seen.set(
      hashState(initialState(level), level),
      JSON.stringify(initialState(level)),
    );
