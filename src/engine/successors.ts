...
import { add, DIRS, eq, inBounds, sub, wallSet } from './grid.ts'
...
export function successors(state: GameState, level: Level): [Input, GameState][] {
  const mechs = active(level)
  const n = level.size
  const canMerge = mechs.some((m) => m.hooks.enablesMerge)
  const resolve = mechs.find((m) => m.hooks.resolveMove)?.hooks.resolveMove ?? stepMove
  ...
  if (!state.merged) {
    ...
    for (const dir of DIRS) {
      ...
      // merge: they meet on the BOARD → a == b + off
      if (canMerge && eq(na, add(nb, state.off)))
        out.push([{ kind: 'move', dir }, { merged: true, m: na, off: state.off }])
      else out.push([{ kind: 'move', dir }, { merged: false, a: na, b: nb, off: state.off }])
    }
  } else {
    ...
    for (const dir of DIRS) {
      ...
      out.push([{ kind: 'move', dir }, { merged: true, m: nm, off: state.off }])
    }
  }

  for (const m of mechs) {
    if (m.hooks.extraSuccessors) out.push(...m.hooks.extraSuccessors(state, level))
  }
  return out
}

export function applyInput(state: GameState, level: Level, input: Input): GameState | null {
