...
export function useProgressSync(uid: string | null, deps: Deps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      //
      // A row seals on `everClean`, the column the server carries for exactly
      // this moment — so a player whose "sans retouche" run was never their
      // best row still gets their seal back here.
