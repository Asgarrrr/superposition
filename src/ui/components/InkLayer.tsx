...
export function InkLayer({ color, pawn, goal, walls, misreg, aligned, size, shift, iceTrail }: {
  color: string
  pawn: Pos
  goal: Pos
  walls: Pos[]
  ...
  return (
    <svg
      ...
      style={{
        transform: aligned ? 'translate(0,0)' : `translate(${misreg}px, ${-misreg}px)`,
    ...
    >
      ...
      >
        ...
        {walls.map(([r, c]) => (
          <g key={`${r}-${c}`} opacity={aligned ? 0.3 : 1} style={{ transition: 'opacity 400ms' }}>
            <rect
              x={PAD + c * CELL + 4}
              y={PAD + r * CELL + 4}
              width={CELL - 8}
