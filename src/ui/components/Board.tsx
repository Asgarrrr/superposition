...
const bumpName = (d: Pos) =>
  d[0] === -1 ? 'sp-bump-U' : d[0] === 1 ? 'sp-bump-D' : d[1] === -1 ? 'sp-bump-L' : 'sp-bump-R'

export function Board({ level, st, solved, bump, bloom, iceTrailA, iceTrailB, onTouchStart, onTouchEnd, children }: {
  level: Level
  st: GameState
  solved: boolean
  bump: Pulse | null
  ...
  const marks: [number, number][] = [[13, 13], [span - 13, 13], [13, span - 13], [span - 13, span - 13]]
  ...
  return (
    <div
      ...
      className="relative aspect-square w-[min(92vw,420px)] overflow-hidden rounded-md"
      style={{
        background: 'radial-gradient(ellipse 80% 80% at 50% 50%, var(--color-box-glow), var(--color-box) 80%)',
        boxShadow: st.merged
    ...
    >
      <InkLayer color="var(--color-ink-cyan)" pawn={pa} goal={level.a.goal} walls={level.a.walls}
        misreg={4} aligned={aligned} size={level.size} shift={[0, 0]} iceTrail={iceTrailA} />
      <InkLayer color="var(--color-ink-magenta)" pawn={pbBoard} goal={level.b.goal} walls={level.b.walls}
        misreg={-4} aligned={aligned} size={level.size} shift={st.off} iceTrail={iceTrailB} />

      <svg viewBox={`0 0 ${span} ${span}`} className="pointer-events-none absolute inset-0 h-full w-full">
        {(level.lightWalls ?? []).map(([r, c]) => {
          const [x, y] = cellCenter([r, c])
          return (
            <g key={`${r}-${c}`} opacity={st.merged ? 1 : 0.4} style={{ transition: 'opacity 400ms' }}>
              <rect x={x - CELL / 2 + 8} y={y - CELL / 2 + 8} width={CELL - 16} height={CELL - 16}
                fill="none" stroke="var(--color-paper)" strokeWidth="2" strokeDasharray="4 4" />
              <circle cx={x} cy={y} r="3" fill="var(--color-paper)" />
            </g>
          )
        })}
        {marks.map(([x, y]) => (
          <RegMark key={`${x}-${y}`} x={x} y={y} offPx={offPx} merged={st.merged} locked={solved} />
        ))}
        {bloom && (
          <circle
