...
import { solveWithout } from "./bfs.ts";
...
describe("hunt — preuves de nécessité", () => {
  it("la preuve fusion retire aussi scission, via la déclaration de la mécanique", () => {
    // the pairing lives on the mechanic declaration, not in the solver
    const removed = MECHANICS.fusion.removedWith;
    expect(removed).toContain("scission");
    // every kept level must be unsolvable once the declared unit is stripped
    const { found } = hunt({ ...EASY, budgetMs: 1500 });
    expect(found.length).toBeGreaterThan(0);
    for (const f of found) expect(solveWithout(f.lv, removed!)).toBeNull();
  });
});
...
describe("hunt — early-exit", () => {
  it("sans stopAtLen épuise le budget ; avec, rend la main bien avant", () => {
    ...
    for (const f of early.found)
      expect(f.len).toBeGreaterThanOrEqual(EASY.minLen);
  });
});
